...
def write_sub(output_dir, src_file, file_list, ofile_name, queue_num):
    ...
    fsrc.write("output = " + os.path.join(output_dir, "out", ofile_name.replace(".pkl", ".out")+" \n"))
    fsrc.write("error = " + os.path.join(output_dir, "err", ofile_name.replace(".pkl", ".err")+" \n"))
    fsrc.write("log = " + os.path.join(output_dir, "log", ofile_name.replace(".pkl", ".log")+" \n"))
...
python ${workdir}/make_emu_batch.py --file_list $1 --out_file $2
'''
    out_script = re.sub('WORKDIR', work_dir, script_template)

    with open(os.path.join(out_dir, 'exe', 'run_python_on_condor.sh'), 'w') as f:
        f.write(out_script)
    os.system('chmod a+x ' + os.path.join(out_dir, 'exe', 'run_python_on_condor.sh')
...
#],
    ...
    file_name_base = 'output_{}_emu_eta1p5_a_'+date+'_{}.pkl'
  
    working_dir = os.getcwd()

    output_dir = os.path.join(working_dir, 'output_condor_tables_'+date)
    ...
    list_of_sam_files = OrderedDict()
    for sample in sample_list:
        list_of_sam_files[sample] = []
        tmp_list = sample_list[sample]['files']

        for i in xrange(0, len(tmp_list), files_per_job):
            list_of_sam_files[sample].append(tmp_list[i:i+files_per_job])

    for sample in list_of_sam_files:
        submit_list = OrderedDict()
        submit_list[sample] = OrderedDict()
        submit_list[sample]['trees'] = sample_list[sample]['trees']
        submit_list[sample]['files'] = None

        src_file = 'hists_{}'.format(sample)
        file_list = '{}_files'.format(sample)

        submit_name = file_name_base.format(sample, '$(Process)')

        queue_num = len(list_of_sam_files[sample])

        write_sub(output_dir, src_file, file_list, submit_name, queue_num)

        for i_set, file_set in enumerate(list_of_sam_files[sample]):
